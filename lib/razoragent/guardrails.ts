...
import { MERCHANT_CATALOG } from './catalog-data';
...
export class GuardrailEngine {
  ...
  public updateConfig(newConfig: Partial<GuardrailPolicyConfig>): void {
    this.config = { ...this.config, ...newConfig };
  }

  public getConfig(): GuardrailPolicyConfig {
    return { ...this.config };
  }
   ...
   */
  public evaluate(cart: CartQuote): PolicyDecision {
    ...
    for (const item of cart.items) {
      const catalogItem = MERCHANT_CATALOG.find((p) => p.id === item.productId);
...
export const globalGuardrailEngine = new GuardrailEngine();
