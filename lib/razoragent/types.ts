...
export interface GuardrailPolicyConfig {
  maxSpendLimitINR: number; // e.g. ₹5,000 max per single agent transaction
  allowedCategories: ProductCategory[];
  maxQuantityPerItem: number;
  requireHumanApprovalAboveINR: number; // e.g. > ₹10,000 requires human signature
  idempotencyWindowSeconds: number; // e.g. 60s
}
...
export interface RazorpayOrderPayload {
  amount: number; // in paise (e.g. ₹3,500 = 350000)
  currency: 'INR';
  receipt: string;
  notes: {
    agent_id: string;
